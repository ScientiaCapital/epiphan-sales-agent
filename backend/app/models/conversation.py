...
class Conversation(Base, TimestampMixin):
    ...
    # Clari Reference
    clari_id: Mapped[Optional[str]] = mapped_column(String(100), unique=True, index=True)

    # Lead/Deal Reference
    lead_id: Mapped[Optional[int]] = mapped_column(Integer, index=True)
    hubspot_deal_id: Mapped[Optional[str]] = mapped_column(String(50), index=True)
    company_name: Mapped[Optional[str]] = mapped_column(String(255), index=True)

    # Participants
    ae_name: Mapped[Optional[str]] = mapped_column(String(100), index=True)
    ae_email: Mapped[Optional[str]] = mapped_column(String(255))
    prospect_name: Mapped[Optional[str]] = mapped_column(String(255))
    prospect_title: Mapped[Optional[str]] = mapped_column(String(255))
    prospect_email: Mapped[Optional[str]] = mapped_column(String(255))
    participant_count: Mapped[int] = mapped_column(Integer, default=2)

    # Call Details
    conversation_type: Mapped[Optional[ConversationType]] = mapped_column(String(30))
    ...
    engagement_score: Mapped[Optional[float]] = mapped_column(Float)  # 0 to 100
    talk_ratio: Mapped[Optional[float]] = mapped_column(Float)  # AE talk time %

    # Extracted Signals
    buying_signals: Mapped[Optional[list]] = mapped_column(ARRAY(String))
    objections: Mapped[Optional[list]] = mapped_column(ARRAY(String))
    competitors_mentioned: Mapped[Optional[list]] = mapped_column(ARRAY(String))
    next_steps: Mapped[Optional[list]] = mapped_column(ARRAY(String))
    pain_points: Mapped[Optional[list]] = mapped_column(ARRAY(String))
    decision_makers: Mapped[Optional[list]] = mapped_column(ARRAY(String))

    # Topics & Keywords
    topics: Mapped[Optional[list]] = mapped_column(ARRAY(String))
    key_phrases: Mapped[Optional[list]] = mapped_column(ARRAY(String))

    # Outcome
    outcome: Mapped[Optional[ConversationOutcome]] = mapped_column(
        String(30), index=True
    )
    outcome_reason: Mapped[Optional[str]] = mapped_column(Text)

    # Deal Impact
    deal_stage_before: Mapped[Optional[str]] = mapped_column(String(50))
    deal_stage_after: Mapped[Optional[str]] = mapped_column(String(50))
    deal_amount: Mapped[Optional[float]] = mapped_column(Float)

    # Full Analysis (JSON blob with detailed AI analysis)
    full_analysis: Mapped[Optional[dict]] = mapped_column(JSONB)

    # Sync Metadata
    synced_from_clari_at: Mapped[Optional[datetime]] = mapped_column(
...
class ConversationInsight(Base, TimestampMixin):
    ...
    context: Mapped[Optional[str]] = mapped_column(Text)  # Surrounding conversation
    response: Mapped[Optional[str]] = mapped_column(Text)  # How AE handled it
    ...
    deal_size_range: Mapped[Optional[str]] = mapped_column(String(50))
    persona: Mapped[Optional[str]] = mapped_column(String(100))  # Decision maker type
